...
const saveRole = () => {
    ...
    if (role?.value.name?.trim()) {
        const form = useForm({
            ...
            email: role.value.email,
            password: role.value.password,
            password_confirmation: role.value.password,
        });
...
const deleteSelectedRoles = () => {
    ...
    let filteredIds = [...new Set([...hasPermissionSelf('roles.self.delete', ids), ...hasPermissionOthers('roles.others.delete', ids)])].filter((value) => value > 1);
    ...
    <div>
        <div class="card mb-4">
            ...
            <Toolbar class="mb-6">
                <template #start>
                    <!--                    create new role-->
                    <Button v-if="hasPermission('roles.others.create')" class="mr-2" icon="pi pi-plus"
                            label="Create Role"
                            @click="createNewRole"/>
                    <Button :disabled="!selectedRoles || !selectedRoles.length" icon="pi pi-trash" label="Delete"
                ...
                <Column header="Image">
                    <template #body="slotProps">
                        <img :alt="slotProps.data.image"
                             :src="`https://primefaces.org/cdn/primevue/images/role/${slotProps.data.image}`"
                ...
                <Column :exportable="false" header="Action" style="min-width: 3rem">
                    <template #body="slotProps">
                        <div class="flex justify-start space-x-2">
                            <AdminButton
                                v-if="slotProps.data.id!==1 && hasPermission(['roles.assign.*','roles.revoke.*'])"
                                :obj="slotProps.data.id"
                                route-name="roles.assignRolePermissionView"
                            >
                            ...
                            <AdminButton
                                v-if="slotProps.data.id!==1 && (hasPermissionSelf('roles.self.edit',slotProps.data.id) || hasPermissionOthers('roles.others.edit',slotProps.data.id))"
                                :obj="slotProps.data.id"
                            ...
                            <AdminButton
                                v-if="slotProps.data.id!==1 && (hasPermissionSelf('roles.self.delete',slotProps.data.id) || hasPermissionOthers('roles.others.delete',slotProps.data.id))"
                                :obj="slotProps.data"
                            ...
                            <AdminButton
                                v-if="hasPermissionOthers('roles.others.login',slotProps.data.id)"
                                :obj="slotProps.data.id"
                                route-method="post"
                                route-name="roles.OthersLogin"
                            >
        ...
        <Dialog v-model:visible="userDialog" :modal="true" :style="{ width: '450px' }" header="Role Details">
            <div class="flex flex-col gap-6">
                <img v-if="role.image" :alt="role.image"
                     :src="`https://primefaces.org/cdn/primevue/images/role/${role.image}`" class="block m-auto pb-4"/>
                <div>
                ...
                <div>
                    ...
                    <InputText
                        id="email" v-model.trim="role.email" :invalid="submitted && !role.email"
                        fluid
                        required="true"/>
                    <small v-if="submitted && !role.email" class="text-red-500">Email is required.</small>
                </div>
                ...
                <div>
                    ...
                    <Password
                        id="password"
                        v-model.trim="role.password" :invalid="submitted && !role.password"
                        fluid
                        required
                        toggleMask/>
                    <small v-if="submitted && !role.password" class="text-red-500">Password is required.</small>
                </div>
